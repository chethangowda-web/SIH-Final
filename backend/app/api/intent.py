...
def submit_intent(
    ...
    # Dispatch Real-Time WhatsApp Confirmation to Citizen Beneficiary
    try:
        from app.services.notification_engine import notification_service
        ...
        wa_message = (
            ...
            f"Namaskara {user_name},\n"
            f"Your PDS Advance Collection Plan has been *REGISTERED* successfully!\n\n"
            f"📋 *Receipt ID:* {request_id}\n"
            f"🗓️ *Cycle:* {payload.cycle_id.strip()} (September 2026)\n"
            f"🏪 *Selected Center:* {intended_fps_name} ({payload.intended_fps_id.strip()})\n"
            f"📦 *Allocated Quota:* {authoritative_qty:.1f} kg {payload.commodity} (₹0.00 FREE)\n"
            f"🚚 *Service Mode:* {mode_str}\n"
            f"💰 *Foodgrain Cost:* ₹0.00 (100% Subsidized)\n\n"
            f"✅ *Status:* Recorded in Pre-Dispatch Demand Plan.\n"
        ...
        logger.info(f"WhatsApp confirmation dispatched to {user_phone} for {request_id}")
    except Exception as wa_err:
        logger.warning(f"Failed to dispatch WhatsApp confirmation: {wa_err}")
...
class WhatsAppReceiptPayload(BaseModel):
    beneficiary_id: str
    request_id: str
    cycle_id: str = "2026-09"
    intended_fps_name: str
    commodity: str = "Rice & Wheat"
    quantity_kg: float = 25.0
    delivery_mode: str = "FPS_COLLECTION"
    phone_number: Optional[str] = None

@router.post("/intent/send-whatsapp-receipt")
def send_whatsapp_receipt(
    ...
    """
    Explicitly dispatch or re-send official PDS WhatsApp collection receipt to beneficiary via Twilio.
    """
    from app.services.notification_engine import notification_service
    from app.core.config import settings
    ...
    wa_message = (
        ...
        f"Namaskara {user_name},\n"
        f"Your PDS Advance Collection Plan has been *REGISTERED* successfully!\n\n"
        f"📋 *Receipt ID:* {payload.request_id}\n"
        f"🗓️ *Cycle:* {payload.cycle_id} (September 2026)\n"
        f"🏪 *Center:* {payload.intended_fps_name}\n"
        f"📦 *Allocated Quota:* {payload.quantity_kg:.1f} kg {payload.commodity} (₹0.00 FREE)\n"
        f"🚚 *Service Mode:* {mode_str}\n"
        f"💰 *Foodgrain Cost:* ₹0.00 (100% Subsidized)\n\n"
        f"✅ *Status:* Recorded in Pre-Dispatch Demand Plan.\n"
        f"You will receive an instant arrival notification when grain arrives at your shop."
    ...
    res = notification_service.send_whatsapp(
        recipient_phone=target_phone,
        recipient_name=user_name,
        message=wa_message,
        ref_id=payload.request_id
    )
    ...
    return {
        ...
        "twilio_status": res.get("status"),
        "delivered_message": wa_message
    }
